...
/**
 * 
 * @param {number} amount Quantity of locations to be generated
 * @returns {Promise<Array>}
 */
module.exports = (amount = 1) => {
  return new Promise((resolve, reject) => {
    if (isNaN(amount)) reject(new TypeError(`Expected a number, got ${typeof amount}`))

    resolve(Array(amount > 0 ? amount : 1).fill().map(() => {
